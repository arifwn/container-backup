    ...
    then
        ...
        for db in $databases; do
            if [[ "$db" != "information_schema" ]] && [[ "$db" != "performance_schema" ]] && [[ "$db" != "mysql" ]] && [[ "$db" != "sys" ]] && [[ "$db" != "Database" ]] ; then
                    ...
                    mysqldump --max_allowed_packet=1073741824 --routines --host $HOST --port $PORT -u$USER -p$PASSWORD $db | gzip -c > $db.sql
            fi
        done

    else
        echo "running mysqldump --max_allowed_packet=1073741824 --routines --host $HOST --port $PORT -u$USER -p$PASSWORD $DBNAME > $DBNAME.sql"
        exec mysqldump --max_allowed_packet=1073741824 --routines --host $HOST --port $PORT -u$USER -p$PASSWORD $DBNAME | gzip -c > $DBNAME.sql
        echo "done"
    fi
fi
